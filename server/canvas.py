...
# ============================================================================
# Exportable functions for use by other modules (e.g., discord_reminder.py)
# ============================================================================

def get_assignments_for_course(course_id):
    """
    Get all assignments for a specific course with pagination.
    Returns raw assignment data.
...
def get_assignments_next_week(course_id=None):
    """
    Get assignments due in the next 7 days.
    This function is designed to be used by other modules like discord_reminder.py.
    
    Args:
        course_id: Optional course ID to filter by (if None, gets from all courses)
    
    Returns:
        List of assignment dictionaries with course_id and course_name added
    """
    ...
    if course_id:
        ...
        courses = get_user_courses()
        # Remove print statement for cleaner output when used as module
        if not courses:
            return []
        ...
        for assignment in assignments:
            ...
            if due_at:
                try:
                    ...
                    due_date_only = due_date_utc.date()
                    
                    # Check if assignment is due in the next 7 days
                    if today_date <= due_date_only < next_week_date:
                        assignment['course_id'] = course_id
                        assignment['course_name'] = course_name
                        all_upcoming.append(assignment)
                except (ValueError, TypeError):
                    continue
    
    # Sort by due date
    all_upcoming.sort(key=lambda x: parser.parse(x.get('due_at', '')) if x.get('due_at') else datetime.max)
